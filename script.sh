...
for((i=0;i<num;i++))
do
curl -d "uid=${uidarr[i]}&upw=${upwarr[i]}&smbtn=$smbtn&hh28=754" -s $url1 -o temp.txt
...
curl -d "@myvs.txt" -d "jingdu=114.15&weidu=22.24&ptopid=$ptopid&sid=$sid" -s $url2 -o temp.txt #打卡
